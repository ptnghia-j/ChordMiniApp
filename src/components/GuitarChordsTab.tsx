    ...
    <div className={`guitar-chords-tab space-y-6 ${className}`}>
      ...
      <div className="chord-diagrams-section">
        ...
        {!isLoadingChords && viewMode === 'animated' ? (
          // Animated View - Enhanced responsive chord diagrams with proper breakpoints
          <div className="animated-chord-view">
            {/* Responsive chord diagram container */}
            <div className="chord-diagrams-container">
              {/* All screen sizes: Horizontal row with responsive spacing */}
                  ...
                  {getVisibleChordRange.map((chordInfo) => {
                    ...
                    return (
                      <div
                        ...
                        className={`flex-shrink-0 transition-all duration-500 ease-in-out rounded-lg ${
                          chordInfo.isCurrent
                            ? 'scale-105 sm:scale-105 md:scale-110 lg:scale-110 z-10 shadow-lg transform'
                            : 'scale-95 sm:scale-100 md:scale-100 opacity-80 sm:opacity-85 transform'
                        }`}
                      >
                        <GuitarChordDiagram
                          chordData={chordData}
                          size={chordInfo.isCurrent ? 'large' : 'medium'}
                          showChordName={true}
                          className="transition-all duration-500 ease-in-out"
                          displayName={chordInfo.chord} // Use corrected chord name for display
                        />
                      </div>
                    );
                  })}
