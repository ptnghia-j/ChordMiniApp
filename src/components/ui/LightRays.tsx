...
const LightRays: React.FC<LightRaysProps> = ({
  ...
  // Store the latest values of props in a ref to avoid re-running the WebGL setup
  ...
  // Keep propsRef in sync on prop changes
  useEffect(() => {
    propsRef.current = {
      raysOrigin,
      raysColor,
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Fade the opacity/intensity towards the bottom instead of darkening to black, preventing dark streaks in light mode
  float fade = 0.15 + brightness * 0.85;
  fragColor.rgb *= fade;
  fragColor.a *= fade;
...
void main() {
  ...
  gl_FragColor  = color;
      ...
      const loop = (t: number) => {
        ...
        if (propsRef.current.followMouse && propsRef.current.mouseInfluence > 0.0) {
      ...
      const handleVisibilityChange = () => {
        if (!document.hidden && !animationIdRef.current && rendererRef.current) {
          animationIdRef.current = requestAnimationFrame(loop);
        }
      };
