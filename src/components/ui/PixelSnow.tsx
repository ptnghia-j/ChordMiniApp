...
const PixelSnow: React.FC<PixelSnowProps> = ({
  ...
  const handleResize = useCallback(() => {
    ...
    resizeTimeoutRef.current = window.setTimeout(() => {
      ...
      const targetWidth = Math.min(w, pixelResolution);
  ...
  // Visibility observer & Tab active observer
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        if (entry.isIntersecting && !wasVisible && startAnimateRef.current) {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        if (!isLoopingRef.current && isVisibleRef.current && startAnimateRef.current) {
          startAnimateRef.current();
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const targetWidth = Math.min(containerWidth, pixelResolution);
    ...
    startAnimateRef.current = () => {
      if (!isLoopingRef.current && isVisibleRef.current && !document.hidden) {
