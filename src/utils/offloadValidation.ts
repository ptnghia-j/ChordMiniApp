 ...
 *
 * Centralises strict URL parsing so every route that touches offload
...
export type OffloadStorageProvider = 'firebase';
...
function detectProviderFromHostname(hostname: string): OffloadStorageProvider | null {
 ...
 */
export function validateOffloadUrl(url: unknown): string {
 ...
 */
export function getOffloadStorageProvider(url: string): OffloadStorageProvider | null {
  try {
