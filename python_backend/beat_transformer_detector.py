...
class BeatTransformerDetector:
    ...
    def detect_beats(self, audio_file):
        ...
        try:
            ...
            beat_time_range_start = float(dbn_beat_times[0]) if len(dbn_beat_times) > 0 else 0.0
            beat_time_range_end = float(dbn_beat_times[-1]) if len(dbn_beat_times) > 0 else duration
