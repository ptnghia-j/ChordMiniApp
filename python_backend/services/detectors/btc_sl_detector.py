...
class BTCSLDetectorService:
    ...
    def is_available(self) -> bool:
        ...
        try:
            ...
            # Check for BTC-SL specific files
            ...
            # Check for specific BTC-SL files
            required_files = [
                "config/btc_model_large_voca_sl.yaml",
                "checkpoints/btc_model_large_voca_sl.pt"
            ]
            
            for file in required_files:
                if not (self.model_dir / file).exists():
                    log_debug(f"BTC-SL file not found: {file}")
                    self._available = False
                    return False
            ...
            try:
                ...
                log_error(f"BTC-SL import failed: {e}")
                self._available = False
                return False
            finally:
    ...
    def recognize_chords(self, file_path: str, chord_dict: str = 'large_voca', **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # Load model
            config_path = self.model_dir / "config" / "btc_model_large_voca_sl.yaml"
            checkpoint_path = self.model_dir / "checkpoints" / "btc_model_large_voca_sl.pt"
